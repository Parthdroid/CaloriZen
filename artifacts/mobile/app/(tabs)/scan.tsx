...
export default function ScanTab() {
  ...
  const startScan = useCallback(() => {
    ...
    tipInterval.current = setInterval(
      () => setTipIndex((p) => Math.min(p + 1, TIPS.length - 1)),
      4000,
    );
  }, [scanLineAnim]);
    ...
    async (uri: string) => {
      ...
      try {
        ...
        const isNonFood =
          items.length === 0 ||
          (items.length === 1 &&
            (analysis.totalCalories ?? 0) === 0 &&
            (items[0]?.name?.toLowerCase().includes("non-food") ||
              items[0]?.name?.toLowerCase().includes("not food")));
        if (isNonFood) {
          stopScan();
          setLoading(false);
          ...
          Alert.alert(
            "No food detected",
            "Try a clearer picture of your meal.",
          );
          return;
        }
        stopScan();
        ...
        Alert.alert(
          ...
          isAbort ? "Please try again." : "Something went wrong.",
        );
      } finally {
        setLoading(false);
        setPreviewUri(null);
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          ...
          "Go to Settings and allow CaloriZen to use your camera.",
        );
        return;
      }
      ...
      Alert.alert(
        ...
        "Your device camera couldn't be opened. Try using Gallery instead.",
      );
    }
  }, [handleImage]);
  ...
  const handleGallery = useCallback(async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Photo access needed",
          "Go to Settings and allow CaloriZen to access your photos.",
        );
        return;
      }
  ...
  if (loading && previewUri) {
    const ty = scanLineAnim.interpolate({
      inputRange: [0, 1],
      outputRange: [0, SH],
    });
    return (
      <View style={[st.loadWrap, { backgroundColor: "#000" }]}>
        ...
        <Animated.View
          style={[
            st.scanLine,
            { backgroundColor: colors.tint, transform: [{ translateY: ty }] },
          ]}
        />
          ...
          <View style={st.viewfinder}>
            ...
            <View style={st.cameraCenter}>
              ...
              <Text style={st.cameraHint}>
                Point camera at any meal or food item
              </Text>
            </View>
          </View>
        </Pressable>
